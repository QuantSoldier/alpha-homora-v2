...
contract UniswapV2ERC20 {
    using MockUniswapV2FactorySafeMath for uint256;
...
contract MockUniswapV2FactoryUniswapV2Pair is UniswapV2ERC20 {
        ...
        returns (bool feeOn)
    {
        ...
        if (feeOn) {
            if (_kLast != 0) {
                uint256 rootK =
                    MockUniswapV2FactoryMah.sqrt(
                        uint256(_reserve0).mul(_reserve1)
                    );
                uint256 rootKLast = MockUniswapV2FactoryMah.sqrt(_kLast);
                if (rootK > rootKLast) {
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        uint256 balance0 =
            MockUniswapV2FactoryIERC20(_token0).balanceOf(address(this));
        uint256 balance1 =
            MockUniswapV2FactoryIERC20(_token1).balanceOf(address(this));
    ...
    function swap(
        ...
        uint256 balance1;
        {
            ...
            balance0 = MockUniswapV2FactoryIERC20(_token0).balanceOf(
                address(this)
            );
            balance1 = MockUniswapV2FactoryIERC20(_token1).balanceOf(
                address(this)
            );
        }
        uint256 amount0In =
    ...
    function skim(address to) external lock {
        ...
        _safeTransfer(
            _token0,
            to,
            MockUniswapV2FactoryIERC20(_token0).balanceOf(address(this)).sub(
                reserve0
            )
        );
        _safeTransfer(
            _token1,
            to,
            MockUniswapV2FactoryIERC20(_token1).balanceOf(address(this)).sub(
                reserve1
            )
        );
    }
...
contract MockUniswapV2Factory {
        ...
        returns (address pair)
    {
        ...
        bytes memory bytecode =
            type(MockUniswapV2FactoryUniswapV2Pair).creationCode;
