...
contract StrategyAddTwoSidesOptimal is Ownable, ReentrancyGuard, Strategy {
    ...
    function execute(
        ...
        /* debt */
        bytes calldata data
    ) external payable override onlyGoblin nonReentrant {
        // 1. Find out what farming token we are dealing with.
        (address fToken, uint256 fAmount, uint256 minLPAmount) =
        ...
        bool isReversed;
        {
            ...
            (uint256 ethReserve, uint256 fReserve) =
        ...
        if (isReversed) {
            ...
            router.swapExactETHForTokens.value(swapAmt)(
                0,
                path,
                address(this),
                now
            ); // ETH to farming tokens
        }
