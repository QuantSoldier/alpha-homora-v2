...
contract BaseKP3ROracle is Initializable {
    ...
    function price0TWAP(address pair) public view returns (uint256) {
        uint256 length = kp3r.observationLength(pair);
        require(length > 0, "no length-1 observation");
        (uint256 lastTime, uint256 lastPx0Cumu, ) =
            kp3r.observations(pair, length - 1);
    ...
    function price1TWAP(address pair) public view returns (uint256) {
        uint256 length = kp3r.observationLength(pair);
        require(length > 0, "no length-1 observation");
        (uint256 lastTime, , uint256 lastPx1Cumu) =
            kp3r.observations(pair, length - 1);
        if (lastTime > now - MIN_TWAP_TIME) {
            require(length > 1, "no length-2 observation");
