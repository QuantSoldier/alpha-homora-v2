...
contract ProxyOracle is IOracle, Governable {
    ...
    function setOracles(address[] memory tokens, Oracle[] memory info)
        external
        onlyGov
    {
        ...
        for (uint256 idx = 0; idx < tokens.length; idx++) {
            require(
                info[idx].borrowFactor >= 10000,
                "borrow factor must be at least 100%"
            );
            require(
                info[idx].collateralFactor <= 10000,
                "collateral factor must be at most 100%"
            );
            require(
                info[idx].liqIncentive >= 10000,
                "incentive must be at least 100%"
            );
            require(
                info[idx].liqIncentive <= 20000,
    ...
    function support(address token, uint256 id)
        external
        view
        override
        returns (bool)
    ...
    function convertForLiquidation(
        ...
        address tokenOutUnderlying =
            IERC20Wrapper(tokenOut).getUnderlyingToken(tokenOutId);
        uint256 rateUnderlying =
            IERC20Wrapper(tokenOut).getUnderlyingRate(tokenOutId);
        ...
        return
            amountOut
                .mul(oracleIn.liqIncentive)
                .mul(oracleOut.liqIncentive)
                .div(10000 * 10000);
    }
    ...
    function asETHCollateral(
        ...
        uint256 ethValue =
            source.getETHPx(tokenUnderlying).mul(amountUnderlying).div(2**112);
